...
class LiveScoreSyncService:
            ...
            try:
                ...
                external = [
                    ExternalMatch(
                        ...
                        stage=stage_key_from_number(item.stage),
                    ...
                    for item in source.load_matches(self.settings.football_data_season)
    ...
    def _match_local(self, external: ExternalMatch, local_matches: list[dict[str, Any]]) -> dict[str, Any]:
        exact_candidates: list[tuple[dict[str, Any], bool]] = []
        for local in local_matches:
            if int(local["stage"]) != external.stage_number:
                continue
            ...
            if not _within_time_window(local.get("played_at"), external.played_at):
                continue
            exact_candidates.append((local, swapped))
        if len(exact_candidates) == 1:
            local, swapped = exact_candidates[0]
            return {"status": "matched", "match": local, "swapped": swapped}
        if len(exact_candidates) > 1:
            return {"status": "ambiguous", "candidate_ids": [item[0]["match_id"] for item in exact_candidates]}
